...
@section('content')
    ...
    <div class="table-responsive">
        <table class="table">
            ...
            @forelse ($today as $a)
                <tr>
                    <td>{{ \Carbon\Carbon::parse($a->start_at)->format('H:i') }} -
                        {{ \Carbon\Carbon::parse($a->end_at)->format('H:i') }}</td>
                    <td>{{ $a->customer->name ?? 'Customer #' . $a->customer_id }}
                        <small>({{ $a->customer->email ?? '' }})</small></td>
                    ...
                    <td>
                        ...
                        @endcan
                        @can('cancel', $a)
                            <form method="POST" action="{{ route('appointments.cancel', $a) }}" class="inline-actions"
                    ...
                    <tr>
                        ...
                        <td>
                            <div class="inline-actions">
                                @can('confirm', $a)
                                    <form method="POST" action="{{ route('appointments.confirm', $a) }}"
                                ...
                                @endcan
                                @can('complete', $a)
                                    <form method="POST" action="{{ route('appointments.complete', $a) }}"
                                        class="inline-actions">
                                        @csrf @method('PATCH')
                                        <button type="submit" class="btn btn-primary btn-sm focus-ring">Complete</button>
                                    </form>
