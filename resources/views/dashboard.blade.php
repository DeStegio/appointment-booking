                    ...
                    <table class="table">
                        ...
                        @foreach ($appointments as $appt)
                            <tr>
                                ...
                                <td>
                                    @if (in_array(strtolower((string) $appt->status), ['pending', 'confirmed'], true) &&
                                            (int) $appt->customer_id === (int) ($user->id ?? 0))
                                        <form method="POST" action="{{ route('appointments.cancel', $appt) }}"
                                            class="inline-actions" data-confirm="Cancel this appointment?">
                                            @csrf
                                            @method('PATCH')
                                ...
                                <td>
                                    <div class="inline-actions">
                                        ...
                                        @if ((int) $appt->provider_id === (int) ($user->id ?? 0) && $status === 'pending')
                                            <form method="POST" action="{{ route('appointments.confirm', $appt) }}"
                                                class="inline-actions">
                                                @csrf
                                                @method('PATCH')
                                                <button type="submit"
                                                    class="btn btn-primary btn-sm focus-ring">Confirm</button>
                                            </form>
                                            <form method="POST" action="{{ route('appointments.cancel', $appt) }}"
                                                class="inline-actions" data-confirm="Cancel this appointment?">
                                                @csrf
                                                @method('PATCH')
                                                <button type="submit"
                                                    class="btn btn-danger btn-sm focus-ring">Cancel</button>
                                            </form>
                                        @elseif ((int) $appt->provider_id === (int) ($user->id ?? 0) && $status === 'confirmed')
                                            <form method="POST" action="{{ route('appointments.complete', $appt) }}"
                                                class="inline-actions">
                                                @csrf
                                                @method('PATCH')
                                                <button type="submit"
                                                    class="btn btn-primary btn-sm focus-ring">Complete</button>
                                            </form>
                                            <form method="POST" action="{{ route('appointments.cancel', $appt) }}"
                                                class="inline-actions" data-confirm="Cancel this appointment?">
                                                @csrf
